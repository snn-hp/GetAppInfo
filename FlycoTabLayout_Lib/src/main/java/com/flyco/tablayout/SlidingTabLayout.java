...
public class SlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    private int mIndicatorStartColor,mIndicatorEndColor;
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        mIndicatorStartColor=ta.getColor(R.styleable.SlidingTabLayout_tl_indicator_start_color,-1);
        mIndicatorEndColor=ta.getColor(R.styleable.SlidingTabLayout_tl_indicator_end_color,-1);
    ...
    private void updateTabStyles() {
        for (int i = 0; i < mTabCount; i++) {
            ...
            if (tvTabTitle!= null){
                tvTabTitle.setTextSize(TypedValue.COMPLEX_UNIT_PX, i == mCurrentTab?mTextSelectSize:mTextUnselectSize);
                ...
                if(i == mCurrentTab){
                    if(mIndicatorStartColor!=-1&&mIndicatorEndColor!=-1){
                        Shader shader = new LinearGradient(
                                0, 0, 0, tvTabTitle.getTextSize(),
                                mIndicatorStartColor,mIndicatorEndColor,
                                Shader.TileMode.CLAMP);
                        tvTabTitle.getPaint().setShader(shader);
                    }else{
                        tvTabTitle.setTextColor(mTextSelectColor);
                    }
                }else{
                    tvTabTitle.setTextColor(mTextUnselectColor);
                    tvTabTitle.getPaint().setShader(null);
                }
            }
        }
    }
    ...
    private void scrollToCurrentTab() {
        ...
        View childView=mTabsContainer.getChildAt(mCurrentTab);
        if(childView==null){
            Log.i("ansen","scrollToCurrentTab childView==null");
            return ;

        }
        int offset = (int) (mCurrentPositionOffset * childView.getWidth());
    ...
    private void updateTabSelection(int position){
        for (int i = 0; i < mTabCount; ++i) {
            ...
            if (tabTitle != null) {
                tabTitle.setTextSize(TypedValue.COMPLEX_UNIT_PX, isSelect?mTextSelectSize:mTextUnselectSize);
                ...
                if(isSelect){
                    if(mIndicatorStartColor!=-1&&mIndicatorEndColor!=-1){
                        Shader shader = new LinearGradient(
                                0, 0, 0, tabTitle.getTextSize(),
                                mIndicatorStartColor,mIndicatorEndColor,
                                Shader.TileMode.CLAMP);
                        tabTitle.getPaint().setShader(shader);
                    }else{
                        tabTitle.setTextColor(mTextSelectColor);
                    }
                }else{
                    tabTitle.setTextColor(mTextUnselectColor);
                    tabTitle.getPaint().setShader(null);
                }
            }
        }
    }
...
//        Log.i("ansen","mIndicatorStyle:"+mIndicatorStyle+" mIndicatorHeight:"+mIndicatorHeight);
...
//            Log.i("ansen","mIndicatorCornerRadius:"+mIndicatorCornerRadius+" mIndicatorColor:"+mIndicatorColor
//            +" mIndicatorStartColor:"+mIndicatorStartColor+" mIndicatorEndColor:"+mIndicatorEndColor);
            if (mIndicatorHeight > 0) {
                if(mIndicatorStartColor!=-1&&mIndicatorEndColor!=-1){
                    int[] colors=new int[]{mIndicatorStartColor,mIndicatorEndColor};
                    mIndicatorDrawable.setColors(colors);

                    int topLeftRadius=50,topRightRadius=50,bottomRightRadius=0,bottomLeftRadius=0;
                    mIndicatorDrawable.setCornerRadii(new float[] {topLeftRadius,topLeftRadius,topRightRadius,topRightRadius,
