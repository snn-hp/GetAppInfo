...
public class TabLayout extends View implements ViewPager.OnPageChangeListener {
    ...
    public TabLayout(final Context context, final AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public TabLayout(final Context context, final AttributeSet attrs, final int defStyleAttr) {
        ...
        final TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.TabLayout);
        try {
            setStripColor(
                    typedArray.getColor(R.styleable.TabLayout_nts_color, DEFAULT_STRIP_COLOR)
            );
            setTitleSize(
                    typedArray.getDimension(R.styleable.TabLayout_nts_size, DEFAULT_TITLE_SIZE)
            );
//            setTitleBold(
//                    typedArray.getBoolean(R.styleable.TabLayout_nts_bold, DEFAULT_TITLE_BOLD)
//            );
            setStripWeight(
                    typedArray.getDimension(R.styleable.TabLayout_nts_weight, DEFAULT_STRIP_WEIGHT)
            );
            setStripFactor(
                    typedArray.getFloat(R.styleable.TabLayout_nts_factor, DEFAULT_STRIP_FACTOR)
            );
            setStripType(
                    typedArray.getInt(R.styleable.TabLayout_nts_type, StripType.LINE_INDEX)
            );
            setStripGravity(
                    typedArray.getInt(R.styleable.TabLayout_nts_gravity, StripGravity.BOTTOM_INDEX)
            );

            setTypeface(typedArray.getString(R.styleable.TabLayout_nts_typeface));
            setInactiveColor(
                    typedArray.getColor(
                            R.styleable.TabLayout_nts_inactive_color, DEFAULT_INACTIVE_COLOR
                    )
            );
            setActiveColor(
                    typedArray.getColor(
                            R.styleable.TabLayout_nts_active_color, DEFAULT_ACTIVE_COLOR
                    )
            );
            setAnimationDuration(
                    typedArray.getInteger(
                            R.styleable.TabLayout_nts_animation_duration, DEFAULT_ANIMATION_DURATION
                    )
            );
            setCornersRadius(
                    typedArray.getDimension(
                            R.styleable.TabLayout_nts_corners_radius, DEFAULT_CORNER_RADIUS
                    )
            );
            ...
            try {
                final int titlesResId = typedArray.getResourceId(
                        R.styleable.TabLayout_nts_titles, 0
                );
